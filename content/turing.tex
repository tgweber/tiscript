...
\emph{Lauf} für $x$ (formal: $l_{x, tm}$) auf einer Turingmaschine $tm$, wenn gilt:
\begin{itemize}
    \item $s_0 = [z_i, \square, tw'(x,0,1), tw'(x,1,|w|-1)]$
    \item $s_{len(S)-1} = [z \in Z, u \in \Gamma^*, v \in \Gamma, w \in \Gamma^*]$
        mit $\delta(z, v)$ undefiniert.
\end{itemize}

\noindent
Ein Lauf $S$ heißt \emph{akzeptierend},
