...
\[
    \phi(L) = \exists n \forall x (
        x \in L
        \wedge  |x| \geq n
        \wedge \exists u \exists v \exists w 
        (
            x = uvw 
...
Er erkrankte aufgrund der Folgen der Hormontherapie an einer Depression
...
\begin{tikzpicture}
	...
	\node[state, below of=z3] (z4) {$z_4$};
	\node[state, accepting, right of=z4] (za) {$z_a$};
	\node[state, left of=z4] (z5) {$z_5$};
	\node[state, left of=z5] (z6) {$z_6$};
	\draw 
        ...
        (z3) edge[right] node{1,$\square$,L} (z4)
        (z4) edge[above] node{$\square$,$\square$,-} (za)
        (z4) edge[above] node{1,1,L} (z5)
