...
\begin{itemize}
    \item Ein \emph{Schnappschuss} s eines DFA A ist ein geordnetes Paar $s = [z, w]$
        ...
        Nehmen wir den DFA von oben, dann wäre $[z_i,0010]$ ein Beispiel:
        Wir sind in Zustand $z_i$ und müssen noch $0010$ lesen.
...
\begin{enumerate}
    ...
    \item Gegeben sei der reguläre Ausdruck $\gamma = (a|b)^*ab$.
        ...
        \end{enumerate}
    \item Gegeben sie der DFA D wie in \autoref{fig:min-advanced} \cite{dfa-minimierung-lmu}.
        \begin{enumerate}
            \item Minimieren Sie D und stellen Sie den minimierten DFA graphisch dar.
            \item Wie lautet das Alphabet $\Sigma$ der Sprache $L(D)$?
        \end{enumerate}
\end{enumerate}
...
\begin{tikzpicture}
	\node[state, initial] (z0) {$z_0$};
	\node[state, above right of=z0] (z1) {$z_1$};
	\node[state, right of=z0] (z2) {$z_2$};
	\node[state, below right of=z0] (z3) {$z_3$};
	\node[state, accepting, right of=z2] (z4) {$z_4$};
	\node[state, accepting, below of=z4] (z5) {$z_5$};
	\draw
	(z0) edge[bend left, above] node{a} (z1)
    (z0) edge[right, above] node{b} (z2)
    (z0) edge[bend right, right] node{c} (z3)
	(z1) edge[loop above] node{a,b} (z0)
	(z1) edge[bend left, above] node{c} (z4)
	(z2) edge[loop above] node{a,b} (z2)
	(z2) edge[right, above] node{c} (z4)
	(z3) edge[loop below] node{a,b} (z3)
	(z3) edge[right, below] node{c} (z5)
	(z4) edge[bend left,right] node{a,b,c} (z5)
	(z5) edge[bend left,left] node{a,b,c} (z4)
    ;
\end{tikzpicture}
\caption{DFA D}
\label{fig:min-advanced}
\end{figure}
