...
\begin{array}{llrlrl}
    ...
    [
        ...
        & [z_{LS},  & \square1\square,          & 1,        & 1]            & , \\
        & [z_{LS},  & \square1\square\square,   & 0,        & \square]      & , \\
...
\begin{tikzpicture}
    %Initial part
	\node[state, accepting, initial] (zI) {$z_I$};

    \node[state, below right of=zI] (zI11) {$z_{I11}$};
    \node[state, below of=zI11] (zI12) {$z_{I12}$};
    \node[state, below left of=zI] (zI01) {$z_{I01}$};
    \node[state, below of=zI01] (zI02) {$z_{I02}$};

    \node[state, below right of=zI02] (zI3) {$z_{I3}$};

    %Loop part
    \node[state, below of=zI3] (zLS) {$z_{LS}$};
    \node[state, below of=zLS] (zLC) {$z_{LC}$};

    \node[state, left of=zLC] (zL01) {$z_{L01}$};
    \node[state, below left of=zL01] (zL02) {$z_{L02}$};

    \node[state, below of=zLC] (zL11) {$z_{L11}$};
    \node[state, below left of=zL11] (zL12) {$z_{L12}$};

    \node[state, left of=zL12] (zL3) {$z_{L3}$};
    \node[state, above of=zL02] (zL4) {$z_{L4}$};
    %Finish part

    \node[state, right of=zLC] (zF0) {$z_{F0}$};
    \node[state, below right of=zF0] (zF1) {$z_{F1}$};
    \node[state, accepting, below of=zF1] (zF2) {$z_{F2}$};

	\draw

        %Initial part
        (zI) edge[above left] node{0,0,L} (zI01)
        (zI01) edge[left] node{$\square$,$\square$,L} (zI02)
        (zI02) edge[below left] node{$\square$,0,R} (zI3)

        (zI) edge[above right] node{1,1,L} (zI11)
        (zI11) edge[right] node{$\square$,$\square$,L} (zI12)
        (zI12) edge[below right] node{$\square$,1,R} (zI3)

        (zI3) edge[right] node{$\square$,$\square$,R} (zLS)

        %Loop part
        (zLS) edge[right] 
            node[text width=1cm, align=center]{0,$\square$,R\\1,$\square$,R} 
            (zLC)


        (zLC) edge[above] node{0,0,L} (zL01)
        (zLC) edge[right] node{1,1,L} (zL11)
        (zLC) edge[above] node{$\square$,$\square$,L} (zF0)

        (zL01) edge[loop above] node{$\square$,$\square$,L} (zL01)
        (zL01) edge[above left]
        ...
        (zL12) edge[loop right] 
            node[text width=1cm, align=center]{0,0,L\\1,1,L} 
            (zL12)
        (zL12) edge[above] node{$\square$,1,R} (zL3)

        (zL3) edge[bend left, left] node{0,0,L} (zL4)
        (zL4) edge[bend left, above] node{0,0,L} (zLS)

        %Finish part
        (zF0) edge[loop above] node{$\square$,$\square$,L} (zF0)
        (zF0) edge[above right] 
            node[text width=1cm, align=center]{0,0,L\\1,1,L} 
            (zF1)
        (zF1) edge[loop right] 
            node[text width=1cm, align=center]{0,0,L\\1,1,L} 
...
\label{fig:tmreverse}
\end{figure}
\url{https://turingmachinesimulator.com/shared/ctjtgaeolz}
