...
\begin{figure}[hb]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[start chain=1 going right,node distance=-1.15mm]
            ...
            \node [on chain=1,tmtape] {$c_{i}$};
            \node [on chain=1,tmtape] {$\ldots$};
            \node [on chain=1,tmtape] {$c_{n}$/$o_{1}$};
            \node [on chain=1,tmtape] {$\ldots$};
            \node [on chain=1,tmtape] (2) {$o_i$};
            \node [on chain=1,tmtape, initial above] (1) {$o_{i+1}$};
            \node [on chain=1,tmtape] {?};
            \node [on chain=1,tmtape] {};
            \node [on chain=1,tmtape,draw=none] {$\ldots$};
        \end{scope}
        \path [draw, -> ] (1.north) -- ++(0,0.35cm) -- ++(-0.75cm,0) -- (2.north); \node{1};
