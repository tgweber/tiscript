...
haben oft einen logischen ``Kern'',
etwas das in unterschiedlichem Gewand wieder- und wiederkehrt.
Diesen Kern bezeichnen wir in diesem Skript als Problem.

Die alltäglichen IT-Aufgaben werden oft als Userstories dokumentiert.
...
diese und ähnliche Aufgaben auf ein Problem zurückzuführen
und diese damit auf einen Blick einzuteilen:
\begin{itemize}
        \item leicht lösbar
        \item schwer lösbar
        \item unlösbar
\end{itemize}
Diese Einteilung ist von großem Interesse für diejenigen IT-Fachkräfte,
die unnötige Arbeit vermeiden
und die besten der bekannten Lösungen für ihre Probleme implementieren wollen. 
Eine Userstory ist jedoch nicht die Form, 
in der das zugrundeliegende Problem am besten identifiziert werden kann
und um die Klassikation nach (Un-)Lösbarkeit zu rechtfertigen. 
Dafür müssen wir die Grundwerkzeuge der theoretischen Informatik nutzen:
\emph{Abstraktion} und \emph{Formalisierung}.
Wie beide exemplarisch anzuwenden sind,
soll in den folgenden Abschnitten dargelegt werden.
...
\noindent
Was müssen alle Maschinen gemein haben, die $f_{MAX}$ berechnen?
...
\end{itemize}
Übertragen auf \textbf{EVEN} müssen wir eine binäre Repräsentation von $n$ finden,
die Ausgabe haben wir ja schon von vorneherein binär gewählt.
Damit wir gefundenen Methoden später von \textbf{EVEN} auf andere Probleme übertragen können,
